...
.check_class <- function(x, class_name, arg) {
  if (!inherits(x, class_name)) {
    stop(sprintf("'%s' must be of '%s' class", arg, class_name), call. = FALSE)
  }
  x
}
...
check_geer_object <- function(x, arg = "object") {
  .check_class(x, class_name = "geer", arg = arg)
}


check_summary_geer_object <- function(x, arg = "x") {
  .check_class(x, class_name = "summary.geer", arg = arg)
}
...
is_working_test <- function(test) {
  identical(test, "working-wald") ||
    identical(test, "working-score") ||
    identical(test, "working-lrt")
}
...
check_working_lrt_allowed <- function(object,
                                      test = "working-lrt",
                                      arg = "object") {
  object <- check_geer_object(object, arg = arg)
