#' Solving Generalized Estimating Equations For Binary Responses Hybrid
#'
#'
...
#'
#' @export
geewa_binary_cor <- function(formula = formula(data),
                             data = parent.frame(),
                             link = "logit",
                             id = id,
                             repeated = NULL,
  ...
  geesolver_fit <- fit_bingee_hybrid(y,
                                     model_matrix,
                                     id,
                                     repeated,
                                     link,
  ...
  if (any(fit$fitted_values > 1 - eps) || any(fit$fitted_values < eps))
    warning("geewa_binary_hybrid: fitted probabilities numerically 0 or 1 occurred",
            call. = FALSE)
