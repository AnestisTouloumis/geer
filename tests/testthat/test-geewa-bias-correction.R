...
test_that("bcgee methods return geer objects and one-step updates match converged fits", {
  ...
  expect_equal(coef(fit_bcgee_naive), coef(fit_one_step_naive), tolerance = 1e-5)
  expect_equal(coef(fit_bcgee_robust), coef(fit_one_step_robust), tolerance = 1e-5)
  expect_equal(coef(fit_bcgee_empirical), coef(fit_one_step_empirical), tolerance = 1e-5)
})
