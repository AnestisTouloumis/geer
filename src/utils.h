...
// Compute lambda_vector[r] = -0.5 * trace( A^{-1} * block_r )
// where block_r = lambda_matrix.rows(r*p, (r+1)*p - 1), using chol or LU.
...
inline void kron_self_matrix_into(arma::mat& kron_d_matrix_d_matrix_i,
  ...
  if (kron_d_matrix_d_matrix_i.n_rows != m * m || kron_d_matrix_d_matrix_i.n_cols != p * p) {
