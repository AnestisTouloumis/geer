...
arma::vec update_beta_robust_hybrid(const arma::vec & y_vector,
  ...
  arma::vec alpha_star_vector =
    - 0.5 * variancemu("binomial", mu_vector) / variance("binomial", mu_vector);
  ...
  arma::vec alpha_star_vector =
    ...
    for(int r = 1; r < params_no + 1; r++) {
      gamma_vector(r - 1) = - 0.5 *
        trace(solve(naive_matrix_inverse, gamma_matrix.rows((r - 1) * params_no, r * params_no - 1)));
    }
    arma::vec ans = beta_vector + solve(naive_matrix_inverse, (u_vector + gamma_vector));
    return ans;
}
...
arma::vec update_beta_empirical_hybrid(const arma::vec & y_vector,
  ...
  arma::vec delta_tilde_star_vector =
    (delta_vector % mueta3(link, eta_vector) - 2 * pow(mueta2_vector, 2))/
      pow(delta_vector, 4);
  arma::vec variance_vector = variance("binomial", mu_vector);
  arma::vec variancemu_vector = variancemu("binomial", mu_vector);
  arma::vec alpha_star_vector = - 0.5 * variancemu_vector / variance_vector;
  arma::vec alpha_tilde_star_vector =
    0.5 * (pow(variancemu_vector, 2)/variance_vector - variancemu2("binomial", mu_vector))/
      variance_vector;
...
arma::vec update_beta_hybrid(const arma::vec & y_vector,
  ...
  if(std::strcmp(type, "gee") == 0){
    ans = update_beta_gee_hybrid(y_vector,
                                 model_matrix,
                                 id_vector,
                                 repeated_vector,
                                 link,
    ...
    ans = update_beta_naive_hybrid(y_vector,
                                   model_matrix,
                                   id_vector,
                                   repeated_vector,
                                   link,
    ...
    ans = update_beta_robust_hybrid(y_vector,
                                    model_matrix,
                                    id_vector,
                                    repeated_vector,
                                    link,
    ...
    ans = update_beta_empirical_hybrid(y_vector,
                                       model_matrix,
                                       id_vector,
                                       repeated_vector,
                                       link,
    ...
    ans = update_beta_jeffreys_hybrid(y_vector,
                                      model_matrix,
                                      id_vector,
                                      repeated_vector,
                                      link,
